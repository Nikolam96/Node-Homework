...
function euroToDollar(euro) {
    let euroConverter =  euro < 0 ? console.log("Please enter a positive number!") : euro * 1.10;
    let fixConverter = euroConverter.toFixed(2)
    console.log(`${arguments[0]} Euro is ${fixConverter} $`)
//  return (`${arguments[0]} Euro is ${fixConverter} $`)
}

function dollarToEuro(dollar) {
    let dollarConverter =  dollar < 0 ? console.log("Please enter a positive number!") : dollar * 0.90;
    let fixConverter = dollarConverter.toFixed(2)
    console.log(`${arguments[0]} $ is ${fixConverter} Euro`)
}

function euroToDenar(euro) {
    let euroConverter =  euro < 0 ? console.log("Please enter a positive number!") : euro * 61.59;
    let fixConverter = euroConverter.toFixed(2)
    console.log(`${arguments[0]} Euro is ${fixConverter} Denara`)
}

function denarToEuro(denar) {
    let denarConverter =  denar < 0 ? console.log("Please enter a positive number!") : denar * 0.016;
...
function dollarToDenar(dollar) {
    let dollarConverter =  dollar < 0 ? console.log("Please enter a positive number!") : dollar * 55.94;
    let fixConverter = dollarConverter.toFixed(2)
    console.log(`${arguments[0]} $ is ${fixConverter} Denara`)
}

function denarToDollar(denar) {
    let denarConverter =  denar < 0 ? console.log("Please enter a positive number!") : denar * 0.018;
    let fixConverter =  denarConverter.toFixed(3)
